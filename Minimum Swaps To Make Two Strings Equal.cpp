...
int minSwapsToMakeEqual(string str1, string str2, int n) {
      ...
      if (str2.length() != n || std::set<char>(str1.begin(), str1.end()) != std::set<char>(str2.begin(), str2.end())) {
    ...
    std::vector<std::pair<char, char>> map_arr;
    ...
    while (i < n) {
        ...
        while (j < n) {
            ...
            if (map_arr[i] == std::make_pair(map_arr[j].second, map_arr[j].first)) {
    ...
    while (i < n) {
        ...
        while (j < n) {
            ...
            if (map_arr[i].second == map_arr[j].first && map_arr[i].first != map_arr[i].second && map_arr[j].first != map_arr[j].second) {
                ...
                std::swap(map_arr[i].first, map_arr[j].first);
